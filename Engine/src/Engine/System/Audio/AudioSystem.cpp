...
AudioSystem::AudioSystem()
{
    ...
    //On alloue un nombre de source d�finis en amont
...
SoundHandler_t AudioSystem::play2DSound(Sound2DComponent& i_sound_2d)
{
    ...
    //R�cup�rer une source disponible
    ALuint source = getFreeSource();

    //On r�cup�re le buffer data lier au fichier
...
SoundHandler_t AudioSystem::play3DSound(Sound3DComponent& i_sound_3d)
{
    ...
    alSourcePlay(source);
    // Stocker la source pour pouvoir la contr�ler plus tard
...
void AudioSystem::loadSound(const GameObject& i_game_object)
{
    for (auto audio_component : i_game_object.getComponents<SoundComponent>() ){
        ...
        if (iter != m_buffers_list.end()) {
            //Buffer trouv� dans la map donc pas besoin de la charger
            continue;
        }
        ALuint buffer = -1;
...
ALuint AudioSystem::getFreeSource()
{
    ...
    // Toutes occup�es les sources sont occuper on vole la plus ancienne
